...
function operationRecalc(operation) {
  ...
  for( var i = 0; i < operation.materials.length; i++ ) {
    ...
    try {
      ...
      materialImpl.error = e.message; // this is actually the error response object
      messages.push({
        id : materialImpl.id,
    ...
    try {
      materialImpl.price = materialController.calculateComplexPrice(materialDef, materialImpl);
    } catch(e) {
      materialImpl.price = 0;
      materialImpl.error = e.message;
    }
